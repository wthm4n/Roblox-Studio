--[[
	Passive.lua
	A peaceful NPC that wants nothing to do with you.

	Behaviors:
	  - Detects players entering FleeRadius
	  - Flees in opposite direction
	  - Searches for cover/obstacles to hide behind
	  - Alerts nearby allied Passive NPCs
--]]

local PathfindingService = game:GetService("PathfindingService")
local PersonalityBase    = require(script.Parent.PersonalityBase)
local Config             = require(script.Parent.Parent.Parent.Shared.Config)

local Passive = setmetatable({}, { __index = PersonalityBase })
Passive.__index = Passive

local CFG = Config.Passive

-- ── Constructor ────────────────────────────────────────────────────────────

function Passive.new(entity: any)
	local self = setmetatable(PersonalityBase.new(entity, CFG), Passive)
	self.Name         = "Passive"
	self._hiding      = false
	self._hideTimer   = 0
	self._alerted     = false
	self._updateTimer = 0
	return self
end

-- ── Interface ──────────────────────────────────────────────────────────────

function Passive:OnUpdate(dt: number)
	self._updateTimer += dt

	-- Run checks every 0.2s not every frame
	if self._updateTimer < 0.2 then return end
	self._updateTimer = 0

	local entity   = self.Entity
	local rootPos  = entity.RootPart.Position
	local nearest, dist = self:_nearestPlayer(rootPos)

	if self._hiding then
		self._hideTimer += 0.2
		-- Peek out after hiding long enough
		if self._hideTimer >= CFG.HideDuration then
			self._hiding    = false
			self._hideTimer = 0
		end
		return
	end

	if nearest and dist <= CFG.FleeRadius then
		-- Alert allies before fleeing
		if not self._alerted then
			self._alerted = true
			self:_alertAllies(rootPos, nearest)
		end

		-- Try to find cover first
		local coverPos = self:_findCover(rootPos, nearest)
		if coverPos then
			self._hiding = true
			self._hideTimer = 0
			entity.Humanoid.WalkSpeed = CFG.FleeSpeed
			entity.Pathfinder:MoveTo(coverPos, function()
				-- Arrived at cover — stop and hide
				entity.Pathfinder:Stop()
			end)
		else
			-- No cover — just flee away
			self:_fleeFrom(nearest)
		end
	else
		-- Player gone — reset alert state and resume patrol
		self._alerted = false
		entity.Humanoid.WalkSpeed = Config.Movement.WalkSpeed
		if entity.FSM:GetState() ~= "Patrol" and entity.FSM:GetState() ~= "Idle" then
			entity.FSM:Transition("Idle")
		end
	end
end

function Passive:OnTargetFound(player: Player)
	-- Passive NPCs don't chase — override FSM transition to Chase
	-- Force it back to Idle/Patrol immediately
	task.defer(function()
		if self.Entity.FSM:GetState() == "Chase" or self.Entity.FSM:GetState() == "Attack" then
			self.Entity.FSM:Transition("Idle")
		end
	end)
end

function Passive:OnDamaged(amount: number, attacker: Player?)
	-- Even when hit, just flee — never fight back
	if attacker then
		self:_fleeFrom(attacker)
	end
end

-- ── Private ────────────────────────────────────────────────────────────────

function Passive:_nearestPlayer(from: Vector3): (Player?, number)
	local Players   = game:GetService("Players")
	local nearest   = nil
	local nearDist  = math.huge

	for _, p in ipairs(Players:GetPlayers()) do
		local char = p.Character
		local root = char and char:FindFirstChild("HumanoidRootPart") :: BasePart
		if root then
			local d = (from - root.Position).Magnitude
			if d < nearDist then
				nearDist = d
				nearest  = p
			end
		end
	end
	return nearest, nearDist
end

function Passive:_fleeFrom(player: Player)
	local entity  = self.Entity
	local root    = entity.RootPart.Position
	local pRoot   = player.Character and player.Character:FindFirstChild("HumanoidRootPart") :: BasePart
	if not pRoot then return end

	local away   = (root - pRoot.Position).Unit
	local dest   = root + away * CFG.FleeRadius
	entity.Humanoid.WalkSpeed = CFG.FleeSpeed
	entity.Pathfinder:MoveTo(dest)
end

-- Find a part to hide behind (between NPC and player)
function Passive:_findCover(from: Vector3, player: Player): Vector3?
	local pChar = player.Character
	local pRoot = pChar and pChar:FindFirstChild("HumanoidRootPart") :: BasePart
	if not pRoot then return nil end

	local toPlayer   = (pRoot.Position - from).Unit
	local bestPos    = nil
	local bestScore  = -math.huge

	for _, part in ipairs(workspace:GetDescendants()) do
		if not part:IsA("BasePart") then continue end
		if part.Anchored == false then continue end
		if part.Size.Y < 3 then continue end -- too short to hide behind

		local d = (from - part.Position).Magnitude
		if d > CFG.HideSearchRadius then continue end

		-- Score: prefer parts that are between NPC and player (in the toPlayer direction)
		local toPartDir = (part.Position - from).Unit
		local dot = toPlayer:Dot(toPartDir)  -- negative = part is behind us (good for hiding)

		-- Hide spot = opposite side of part from player
		local awayFromPlayer = (part.Position - pRoot.Position).Unit
		local hidePos = part.Position + awayFromPlayer * (part.Size.X * 0.5 + 3)
		hidePos = Vector3.new(hidePos.X, from.Y, hidePos.Z)

		local score = -dot + (1 / (d + 1))  -- prefer parts NOT toward player
		if score > bestScore then
			bestScore = score
			bestPos   = hidePos
		end
	end

	return bestPos
end

-- Alert other Passive NPCs nearby
function Passive:_alertAllies(from: Vector3, threat: Player)
	for _, model in ipairs(workspace:GetDescendants()) do
		if not model:IsA("Model") then continue end
		if model == self.Entity.NPC then continue end
		if not model:GetAttribute("IsNPC") then continue end
		if model:GetAttribute("Personality") ~= "Passive" then continue end

		local root = model:FindFirstChild("HumanoidRootPart") :: BasePart
		if not root then continue end

		local dist = (from - root.Position).Magnitude
		if dist <= CFG.AllyAlertRadius then
			-- Signal via attribute — each NPC's Passive personality checks this
			model:SetAttribute("AlertedBy", threat.Name)
			task.delay(0.5, function()
				model:SetAttribute("AlertedBy", nil)
			end)
		end
	end
end

function Passive:Destroy() end

return Passive
